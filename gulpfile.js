...
var site_url = "https://renmuell.github.io/de/";
//var site_url = "http://localhost/de/";
//var site_url = "http://192.168.178.38/de/";
...
var path = {
  src : {
    ...
    pages:{
      ...
      me:{
        jade: 'src/jade/ich.jade',
      },
      articleTemplate:{
        jade: 'src/jade/beitrag.jade',
      },
      settings:{
        jade: 'src/jade/einstellungen.jade',
        js: 'assets/js/einstellungen.js'
      }
    },
...
gulp.task('json-render-article', ['json-article'], function() {
  gulp.src('./beitraege.json')
  .pipe(jsonTransform(function(data) {
    for (let index = 0; index < data.articles.length; index++) {
      var article = data.articles[index];
     
      article = jade.compileFile("src/jade/index/beitrag.jade")({
        article: article,
        assetsRoot: "assets/",
        site_url: site_url,
        short: true
...
gulp.task('jade-index', ['json', 'less', 'js', 'copy', 'fonts'], function() {
  ...
  var jsonArticleHtml = require('./beitraege-html.json');

  gulp.src(path.src.pages.index.jade)
    .pipe(data(function(){
        var data = require('./beitraege.json');
...
gulp.task('jade-articles', ['json', 'less', 'copy', 'fonts'], function(){
  var json = require('./beitraege.json');
...
gulp.task('sitemap', [
  ...
  gulp.src('./**/*.html', {
      ...
      .pipe(sitemap({
          siteUrl: 'https://renmuell.github.io/de/'
      }))
      .on('error', onError)
      .pipe(gulp.dest(path.build.root));
});
